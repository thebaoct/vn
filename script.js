...
function renderAll(skipCalc=false) {
    ...
    const final = (billCurrent + oldDebt) - cred; // Tổng phải thu (Đã trừ tiền ví)
        ...
        // Tính toán chuẩn chỉnh và ép số
const rCustGiven = document.getElementById('rowCustomerGiven');
const rChange = document.getElementById('rowChange');
const rRemDebt = document.getElementById('rowRemainingDebt');


if(rPre) rPre.style.display="none"; // Ẩn cái giao diện trả trước cũ đi


if(rCustGiven && rChange && rRemDebt) {
    if(pre > 0) {
        rCustGiven.style.display = "flex";
        document.getElementById('outCustomerGiven').innerText = formatMoney(pre) + " đ";
        if(pre > final) {
            rChange.style.display = "flex";
            document.getElementById('outChange').innerText = formatMoney(pre - final) + " đ";
            rRemDebt.style.display = "none";
        } else if (pre < final) {
            rChange.style.display = "none";
            rRemDebt.style.display = "flex";
            document.getElementById('outRemainingDebt').innerText = formatMoney(final - pre) + " đ";
        } else {
            rChange.style.display = "none";
            rRemDebt.style.display = "none";
        }
        ...
        rChange.style.display = "none";
        if (final > 0 && currentPaymentMethod !== 'CASH' && !isPaid) {
            rRemDebt.style.display = "flex";
            document.getElementById('outRemainingDebt').innerText = formatMoney(final) + " đ";
        } else {
            rRemDebt.style.display = "none";
        }
    }
...
if (bankRow) {
    if (currentPaymentMethod === 'TRANSFER') {
        ...
        document.getElementById('outBankNameDetails').innerText = sel ? sel.options[sel.selectedIndex].text : storeConfig.bankId;
        document.getElementById('outBankAccName').innerText = storeConfig.accName;
    } else {
...
function renderQR(amt=null) {
    ...
    // ĐOẠN FIX LỖI: Cập nhật thêm tên Chủ Tài Khoản vào màn hình in
    if(document.getElementById('qrAccName')) {
        document.getElementById('qrAccName').innerText = `Chủ TK: ${storeConfig.accName}`;
    }
}
...
if(currentPaymentMethod==='DEBT' || remain > 0) {
